...
async def setup_channels(bot: commands.Bot, guild: discord.Guild) -> None:
    ...
    config_set("channel_standings", str(standings_ch.id))

    if not config_get("poll_type"):
        config_set("poll_type", POLL_AP)
    if not config_get("picks_reveal"):
        config_set("picks_reveal", "1")
...
async def _ensure_admin_panel(bot: commands.Bot, channel: discord.TextChannel) -> None:
    ...
    msg_id = config_get("admin_panel_msg_id")
    season = get_active_season()
    week   = get_latest_week(season["id"]) if season else None
    poll_type = config_get("poll_type", POLL_AP)
    ...
    config_set("admin_panel_msg_id", str(msg.id))
...
async def _ensure_picks_hub(bot: commands.Bot, channel: discord.TextChannel) -> None:
    ...
    msg_id = config_get("picks_hub_msg_id")
    season = get_active_season()
    week   = get_latest_week(season["id"]) if season else None
    ...
    msg = await channel.send(embed=embed, view=view)
    config_set("picks_hub_msg_id", str(msg.id))
...
async def _ensure_standings_panels(bot: commands.Bot,
    ...
    season = get_active_season()
    season_rows = get_season_leaderboard(season["id"]) if season else []
    ...
    week_msg_id   = config_get("standings_week_msg_id")
    season_msg_id = config_get("standings_season_msg_id")
    ...
    if week_msg_id:
        try:
            ...
            config_set("standings_week_msg_id", str(msg.id))
    else:
        msg = await channel.send(embed=week_embed)
        config_set("standings_week_msg_id", str(msg.id))
    ...
    if season_msg_id:
        try:
            ...
            config_set("standings_season_msg_id", str(msg.id))
    else:
        msg = await channel.send(embed=season_embed)
        config_set("standings_season_msg_id", str(msg.id))
